...
module.exports = ctx => ({
  // dest: './_site',
  base: '/notes-vuepress/',
  port: 4000, // just for dev server
  locales: {
    '/': {
      lang: 'en-US',
      title: 'notes-vuepress',
      description: 'notes-vuepress Docs'
    },
    '/zh/': {
      lang: 'zh-CN',
      title: 'notes-vuepress',
      description: 'notes-vuepress 手册'
    }
  },
  head: [
  ...
  themeConfig: {
    ...
    locales: {
      '/': {
        ...
        sidebar: {
          //'/api/': getApiSidebar(),
          '/notes-vuepress/': getCmdrGuideSidebar('Contents', 'Introduction', 'Appendix', 'Others'),
      ...
      '/zh/': {
        ...
        sidebar: {
          ...
          '/zh/notes-vuepress/': getCmdrGuideSidebar('正文', '介绍', '附录', '其他'),
...
const cmdrGuideFiles = fs
  .readdirSync(path.resolve(__dirname, '../notes-vuepress/book'))
  .filter(n => !/^\..*/.test(n) && /\.md/.test(n))
  .sort()
  .map(filename => {
      const name = filename.slice(0, -3)
      return (name === 'README') ? 'book/' : 'book/' + name
  })
// console.log(cmdrGuideFiles)

function getCmdrGuideSidebar (title, intro, appendix, others) {
  return [
    {
      title: 'notes-vuepress',
      collapsable: false,
